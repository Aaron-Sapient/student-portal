...
// Stamp the week's check-in on the roster row. Replaces the Master `AY${rowIndex}`
// / `BA${rowIndex}` cell writes, and with them the whole row-index hazard: this is
// a keyed update, so there is no scan position that can go stale across a row
// insert and land one student's stamp on another student's row.
//
// `which` is 'ryan' | 'aaron' | 'both' ('both' = a senior, whose unified weekly
// check-in has always moved BOTH columns so the Friday reminder does not nudge
// them for an Aaron check-in they do not have).
//
// THROWS on failure. AY is what the portal reads as "you're checked in this week",
// and a silent miss leaves the student re-prompted forever.
export async function stampCheckin(studentSheetId, which, atISO) {
  if (!studentSheetId) throw new Error('stampCheckin: missing studentSheetId')
  const patch = {}
  if (which === 'ryan' || which === 'both') patch.last_ryan_checkin = atISO
  if (which === 'aaron' || which === 'both') patch.last_aaron_checkin = atISO
  const { error } = await getSupabaseClient()
    .from('students')
    .update(patch)
    .eq('student_sheet_id', studentSheetId)
  if (error) throw new Error(`stampCheckin(${which}) failed: ${error.message}`)
}

// Clear a check-in stamp, un-checking the student for the week. The inverse of
// stampCheckin; see app/api/resetFormStatus for why this exists at all. Resolves
// by EMAIL because the caller is the signed-in student. Returns false when the
// email owns no active student row.
export async function clearCheckin(email, which) {
  const student = await getStudentByEmail(email)
  if (!student) return false
  const patch = {}
  if (which === 'ryan' || which === 'both') patch.last_ryan_checkin = null
  if (which === 'aaron' || which === 'both') patch.last_aaron_checkin = null
  const { error } = await getSupabaseClient()
    .from('students')
    .update(patch)
    .eq('student_sheet_id', student.student_sheet_id)
  if (error) throw new Error(`clearCheckin(${which}) failed: ${error.message}`)
  return true
}
