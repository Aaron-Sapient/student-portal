...
// Every `students` column an identity consumer reads. ONE list so the flagged
// resolveIdentity path and the flag-free helpers below can never drift apart.
const STUDENT_COLS =
...
async function resolveIdentityFromSupabase(email) {
  ...
  const { data: studs, error: sErr } = await sb
    ...
    .eq('status', 'active')
    .limit(1)
  // THROW (not return null) on a read error: a Supabase blip must be
  // distinguishable from a clean "email not in roster" so the auth-critical
  // `on` path can fall back to Sheets instead of silently locking a real user
  // out. A clean empty result is authoritative (mirror is reconciled ≤10min).
...
// ── Flag-free Supabase identity (ruling 2026-08-27) ─────────────────────────
// Supabase is the record of truth; Google is legacy input, never a runtime
// dependency. A route that reads the Master or a student sheet for identity is a
// defect, not a pending flip — so the check-in routes call these DIRECTLY: no
// `roster` flag, no Sheets fallback, no `sheets` argument. Trigger: Ryan Koo
// could not check in on 2026-08-26 because his sheet lacked the 🔎 Overview tab
// while his row already sat in `student_profiles`.
...
// status='nc' matches the Sheets isNC() exclusion). THROWS on a read error so a
// Supabase blip is never mistaken for "not a student".
...
export async function resolveIdentity(sheets, email) {
  ...
  if (mode === 'on') {
    // Supabase-first, Sheets-fallback-on-ERROR. Identity is auth-critical, so a
    // transient Supabase failure must degrade to the proven Sheets path, never
    // resolve a real user to role:null (a silent lockout). A clean miss is NOT
    // an error and does not fall back (the reconciled mirror is authoritative).
    try {
      return await resolveIdentityFromSupabase(email)
    } catch (e) {
      console.warn(`[roster:supabase] resolveIdentity fell back to Sheets: ${e?.message}`)
...
async function studentBySheetIdFromSupabase(sheetId) {
  ...
  const { data, error } = await sb
    ...
    .limit(1)
  if (error) throw new Error(`studentBySheetId failed: ${error.message}`) // see resolveIdentityFromSupabase
...
export async function studentBySheetId(sheets, sheetId) {
  ...
  if (mode === 'on') {
    try {
      return await studentBySheetIdFromSupabase(sheetId)
    } catch (e) {
      console.warn(`[roster:supabase] studentBySheetId fell back to Sheets: ${e?.message}`)
...
async function listStudentsFromSupabase() {
  ...
  if (error) throw new Error(`listStudents failed: ${error.message}`) // see resolveIdentityFromSupabase
  return (data || [])
...
export async function listStudents(sheets) {
  ...
  if (mode === 'on') {
    try {
      return await listStudentsFromSupabase()
    } catch (e) {
      console.warn(`[roster:supabase] listStudents fell back to Sheets: ${e?.message}`)
