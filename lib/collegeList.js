...
// "28%" / "2.5%" / "0.28" → 0..1 fraction; anything non-numeric → null. Guard:
// a date serial that landed in a percent-formatted cell renders as e.g. "4530000%"
// (serial 45300) — orders of magnitude past any real progress/acceptance value, so
// clamp absurd magnitudes to null rather than show "4530000%".
const pct = (c) => {
  const raw = text(c)
  if (!raw) return null
  const n = Number(raw.replace('%', ''))
  if (Number.isNaN(n)) return null
  const f = raw.includes('%') ? n / 100 : n
  return f < 0 || f > 2 ? null : f
}
...
// 📆 Meetings dates are entered as M/d/yy ("4/28/23") or M/d/yyyy ("4/2/2024"),
// depending on the cell's date format — most older logs are 2-digit. Try both;
// a 2-digit year pivots to the 2000s (Luxon's default cutoff), correct for our
// 2023+ records. (M/d/yyyy alone — the prior code — returned null on every
// 2-digit date, blanking ~69% of meeting dates.)
export const parseMeetingDate = (raw) => {
...
// The 'Date' header usually sits in col B, but ~8 students' grids start the table
// at col A — so locate the header ROW *and* COLUMN within the first few rows and
// read every field relative to that base column (a rigid col-B lookup returned 0
// meetings for those students). And SKIP blank-date rows rather than `break` on
// the first one: some logs have a mid-table empty row, and an early break silently
...
export function parseMeetingsGrid(rows) {
  ...
  for (let r = 0; r < Math.min(rows.length, 4); r++) {
    const col = cells(rows[r]).findIndex((c) => text(c).toLowerCase() === 'date')
    if (col >= 0) {
      hdr = r
      base = col
      break
  ...
  for (let i = hdr + 1; i < rows.length; i++) {
    const c = cells(rows[i])
    const rawDate = text(c[base])
    if (!rawDate) continue
    const dt = parseMeetingDate(rawDate)
    out.push({
      date: dt ? dt.toISODate() : null,
      teacher: text(c[base + 1]) || null,
      project: text(c[base + 2]) || null,
      agenda: text(c[base + 3]) || null,
      homework: text(c[base + 4]) || null,
      hwStatus: text(c[base + 5]).toLowerCase() || null,
      pct: pct(c[base + 6]),
    })
  }
  return out
