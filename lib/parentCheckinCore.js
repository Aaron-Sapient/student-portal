...
// Core of the parent check-in flow, shared by the public /api/parentCheckin
// form and the authenticated /api/parent/checkin portal route. Looks up the
// student (unless the caller already verified one), runs the Haiku urgency
// analysis, appends to the ParentCheckins tab, and emails the support inbox.
...
export async function processParentCheckin({ parentEmail, concern, knownStudentName = null, knownSheetId = null }) {
  ...
  const studentRow = masterRows.find(r => {
    const colK = String(r?.[10] ?? '').trim().toLowerCase();
    const colL = String(r?.[11] ?? '').trim().toLowerCase();
    const nameMatch = knownStudentName
      ? String(r?.[0] ?? '').trim() === knownStudentName
      : true;
    return nameMatch && (colK === target || colL === target);
  });

  let studentName = knownStudentName || studentRow?.[0] || null;
...
Return ONLY a valid JSON object, no markdown, no explanation:
{
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        nowISO,                                                       // A: server-generated
        // B is unverified input from an ANONYMOUS internet POST (this path is public
        // by design); C–H are model-authored off that same input. All of it is
        // untrusted text landing in the Master Sheet under USER_ENTERED.
        sheetSafe(parentEmail),                                       // B
        sheetSafe(studentName),                                       // C
        sheetSafe(parsed.purpose || ''),                              // D
        sheetSafe(parsed.deadlines || 'N/A'),                         // E
        daysSinceLastRequest !== null ? daysSinceLastRequest : '',    // F: number, keeps its typing
        sheetSafe(parsed.urgencyLevel || ''),                         // G
        sheetSafe(parsed.reasoning || ''),                            // H
      ]],
    },
  });

  // ── 6b. Best-effort dual-write to the Supabase mirror (Bucket-A cutover) ───
  // NON-authoritative: a failure here must NEVER abort the Sheet write or email.
  // Keyed identically to scripts/backfillParentCheckins.cjs (same normalization,
  // same submitted_at = nowISO) so the natural key dedupes the two writers. The
  // student_sheet_id FK is NOT NULL → skip (don't attempt) when unresolved; the
  // reconcile backfill will pick it up later if the parent becomes resolvable.
  const pcSheetId = knownSheetId
    || (String(studentRow?.[6] ?? '').match(/\/d\/([a-zA-Z0-9-_]+)/)?.[1] ?? null);
  if (pcSheetId) {
    ...
    try {
      const { error } = await getSupabaseClient()
        ...
        .upsert({
          ...
          parent_email: String(parentEmail ?? '').trim().toLowerCase() || null,
          submitted_at: nowISO,
      ...
      if (error) console.warn('[dual-write:parent_checkins] upsert failed:', error.message);
    } catch (e) {
      console.warn('[dual-write:parent_checkins] skipped:', e?.message || e);
    }
  } else {
    console.warn('[dual-write:parent_checkins] unresolved student_sheet_id — mirror skipped (reconcile backfills if resolvable).');
  }
