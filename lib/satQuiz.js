...
// Content shape: an ordered jsonb array of question items (order is authored &
// preserved — not shuffled — so a warm-up → apply → synthesize sequence stays
// intact). Item types:
//   - 'classify'   : { word, answer }             → "Is <word> a verb?" (Verb / Not a verb)
//   - 'fill'       : { sentence, options, answer } → pick the form that fits the blank
//   - 'odd'        : { options, answer, prompt? }  → pick the odd one out; prompt
//                                                    defaults to the verb-confusion framing
//                                                    but can be overridden per item
//   - 'identify'   : { choices, options, answer }  → 4 read-only reference choices,
//                                                    then pick which category they test
//   - 'first_noun' : { instructions?, parts }      → parts: [{ id, sentence, answer }];
//                                                    one dropdown per sentence, scored
//                                                    as a fraction (1/parts.length each)
// answer ∈ {'verb','not_verb'} for classify; the literal option text for
// fill/odd/identify; the literal word text (per-part) for first_noun.
...
const IDENTIFY_PROMPT = 'Which question type does this test?'

const FIRST_NOUN_PROMPT = 'Select the first noun in each sentence.'

function tokenizeWords(sentence) {
  return (String(sentence || '').match(/[A-Za-z']+/g) || [])
}
...
export function buildGrammarQuiz(content) {
  const questions = (content || []).map((item) => {
    const base = { id: item.id, target: item.id, type: item.type, badge: item.badge || null }
    ...
    if (item.type === 'first_noun') {
      const parts = (item.parts || []).map((p, i) => ({
        id: p.id || String(i),
        sentence: p.sentence,
        options: tokenizeWords(p.sentence),
      }))
      return {
        ...base,
        prompt: item.instructions || FIRST_NOUN_PROMPT,
        promptLabel: 'First noun',
        parts,
      }
    }
    ...
    if (item.type === 'identify') {
      ...
      return {
        ...base,
        prompt: item.prompt || IDENTIFY_PROMPT,
        promptLabel: 'Identify the question type',
    ...
    return {
      ...base,
      prompt: item.type === 'fill' ? item.sentence : (item.prompt || ODD_PROMPT),
...
// [{ target, selectedKey }] for classify/fill/odd/identify, or
// [{ target, parts: { [partId]: word } }] for first_noun. Returns a single
// integer grammar_score + total (a first_noun item counts as `parts.length`
// gradable sub-questions, one point each, rather than a fractional single
// question — keeps both score and total whole numbers, since sat_attempts
// stores scores in an `int` column).
export function scoreGrammarResponses(content, responses) {
  ...
  const answers = items.map((item) => {
    ...
    if (item.type === 'first_noun') {
      ...
      const selectedParts = r.parts && typeof r.parts === 'object' ? r.parts : {}
      let correctCount = 0
      const partResults = parts.map((p) => {
        const selected = selectedParts[p.id] || null
        const ok = selected != null && selected === p.answer
        if (ok) correctCount++
        return { id: p.id, sentence: p.sentence, selected, correct: p.answer, ok }
      })
      grammar_score += correctCount
      total += parts.length

      return {
        id: item.id,
        type: item.type,
        title: item.instructions || FIRST_NOUN_PROMPT,
        detail: null,
        parts: partResults,
        correct: parts.length > 0 && correctCount === parts.length,
      }
    }
    ...
    if (item.type === 'classify') {
      ...
      title =
        item.type === 'fill'
          ? item.sentence
          : item.type === 'identify'
            ? item.prompt || IDENTIFY_PROMPT
            : 'Odd one out — which is the verb?'
      if (item.type === 'odd') detail = opts.join('   ·   ')
      if (item.type === 'identify') detail = (item.choices || []).join('   ·   ')
    }
  ...
  return { grammar_score, total, answers }
}
