...
export async function sendRyanMeetingRequestEmail({ studentName, reason, suggestedLength, signals, tokens, to = RYAN_APPROVAL_EMAIL }) {
...
// Sent to the student (CC both parents) once Ryan grants a meeting, with the
// booking link. Not sent on reject.
export async function sendMeetingGrantedEmail({ studentEmail, parentEmails, studentName, decision }) {
  const transporter = buildTransporter();
  const lengthLabel = decision === '30min' ? '30-minute Zoom' : '15-minute call';
  const bookingUrl = `${baseUrl()}/meetings/ryan`;
  const firstName = (studentName || '').split(' ')[0] || 'there';
  const cc = (parentEmails || []).filter((e) => e && String(e).includes('@'));

  const text = `Hi ${firstName},

Ryan has set up a ${lengthLabel} with you based on your latest check-in. Pick a time that works:
...
— Admissions.Partners`;
  ...
  const html = `
    <div style="max-width:520px;margin:0 auto;font:400 15px/1.6 -apple-system,Segoe UI,Roboto,sans-serif;color:#2b2622">
      ...
      <p style="margin:0 0 18px">Ryan has set up a <strong>${lengthLabel}</strong> with you based on your latest check-in. Pick a time that works:</p>
  ...
  await transporter.sendMail({
    ...
    subject: `Ryan set up a meeting with you — book your time`,
    text,
    html,
  });
}
