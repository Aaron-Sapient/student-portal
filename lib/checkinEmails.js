...
//
// `oneoffId` is REQUIRED whenever the grant being announced is a senior one-off.
// A student can hold an active weekly cross-meeting with the same teacher at the
// same time, and a bare /meetings/<slug> falls back to `mine || oneoffs[0]`
// (app/api/validateBooking/route.js), so the page routes to the weekly cross and
// offers ITS lengths and ITS window — the one-off this email is about is then
// unreachable from its own link. The `?m=oneoff:<id>` key commits the page to
// exactly this grant. Omit it for a booking-token grant, which has no id and no
// competing track.
export async function sendMeetingGrantedEmail({
  ...
  const bookingUrl = oneoffId
    ? `${baseUrl()}/meetings/${instructorSlug}?m=${encodeURIComponent(`oneoff:${oneoffId}`)}`
    : `${baseUrl()}/meetings/${instructorSlug}`;
