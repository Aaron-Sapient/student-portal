...
// ── Reschedule timing (project meetings only) ────────────────────────────────
// Project meetings are exempt from the 24-hour reschedule notice every other track
// keeps, because the case this serves is a student who wakes up sick on the day
// (2026-08-24 · Aaron). In its place, three numbers — defined ONCE here because they are
// enforced in two places that must never disagree: the slot list that decides what a
// student is OFFERED (app/api/getAvailableSlots) and the route that decides what is
// ACCEPTED (app/api/rescheduleProjectMeeting). Duplicated literals in those two files
// would agree on the day they were written and silently drift afterwards.
export const SOURCE_LOCK_HOURS = 2;        // the meeting being moved is frozen inside this
export const DEST_FLOOR_HOURS = 2;         // the new slot must be at least this far out
export const SAME_DAY_EARLIEST_HOUR = 16;  // a move landing TODAY starts no earlier than 4pm

// Is `currentStart` still movable, i.e. outside the source lock? Inside it, the meeting is
// frozen exactly as it is today (the client hides both buttons; the route re-checks).
export function projectSourceUnlocked(currentStart, now) {
  const s = currentStart?.setZone(ZONE);
  return !!s?.isValid && s >= (now || DateTime.now()).setZone(ZONE).plus({ hours: SOURCE_LOCK_HOURS });
}

// The earliest instant a project RESCHEDULE may land on `day`. Two floors, whichever is
// later: now + 2h, and — only when `day` is today — 16:00 local. `day` and `now` are
// compared as LA calendar DATES, never as instants, so this stays correct across the
// November DST shift and for a student browsing from another timezone.
export function projectRescheduleFloor(day, now) {
  const n = (now || DateTime.now()).setZone(ZONE);
  const d = day.setZone(ZONE);
  let floor = n.plus({ hours: DEST_FLOOR_HOURS });
  if (d.toISODate() === n.toISODate()) {
    const earliest = d.set({ hour: SAME_DAY_EARLIEST_HOUR, minute: 0, second: 0, millisecond: 0 });
