...
export async function resolveActor() {
  ...
  if (ADMIN_EMAILS.includes(email)) {
    const name = teacherForEmail(email) ?? 'Admin'
...
// Like resolveActor, but for the DOC-level writing routes (doc/save/tab/history)
// that are keyed by an unguessable doc/tab UUID. When there's a Clerk session we
// resolve the full identity (so attribution + role checks are exact). When there
// ISN'T, we fall back to an anonymous "link" actor: possession of the UUID is the
// capability — anyone with the /write/<docId> link can view + edit, no account or
// login. (Aaron's call, 2026-06-19: "widen the moat now, shrink it later" — the
// existential issue was seniors getting locked out of their essays when the
// identity/college-list chain broke.) Edits are attributed in version history as
// coming via the shared link. A real Google Sheets client (service identity) lets
// the doc route resolve the owning student's name; editor_email is a non-null
// sentinel because md_tab_revisions.editor_email is NOT NULL.
//
// NOTE: this is intentionally NOT used by the student-MAP route (/api/writing,
// resolveViewTarget) — that one is keyed by the (semi-enumerable) Google Sheet
// id, so it stays behind a real session. The no-login flow only ever opens a
// direct /write/<docId> link, which needs the doc routes, not the map.
export async function resolveActorOrLink() {
  const { userId } = await auth()
  if (userId) return resolveActor()
  return {
    role: 'link',
    email: 'link@portal',
    name: 'Student (via link)',
    ownSheetId: null,
...
export function canEditStudent(actor, studentSheetId) {
  ...
  // 'admin' (Aaron/Ryan) and 'link' (anyone holding the doc's UUID) may edit any
  // student's docs; a logged-in student only their own sheet.
  if (actor.role === 'admin' || actor.role === 'link') return true
