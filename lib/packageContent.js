...
// "a" vs "an" for a bonus name. Four of the twelve above start with a vowel
// sound — Early-Bird, AP-Season ("ay-pee"), End-of-Summer, End-of-Year — so a
// hardcoded "a" was wrong a third of the year, most recently in August's "a
// End-of-Summer discount" (Ryan, 2026-08-11). A plain vowel-letter test is
// correct for every value in MONTH_BONUS; it is not a general English rule, so
// keep it pointed at these names.
...
// applies (Menu!D26). `bonusText` is the $-value of the bonus when paired with
// `topLabel`, the most expensive tier actually being offered.
// Three values here used to be hardcoded and were wrong every time the sheet's
// assumptions moved (fixed 2026-08-10): the enrol-by date was the literal
// "4/27" no matter what expiry Ryan set; the duration read "4 to 5 months"
// while the bonus line in the same email printed the real figure in weeks
// (Olivia's July proposal: "4.9 weeks"), so one proposal contradicted itself;
// and the closing clause named VIP even when VIP was not on offer.
export function earlyStartParagraph({ first, grade, gender }, { expiryText, durationText, topLabel, bonusText }) {
  const p = pronouns(gender)
  return (
    `Officially, our ${grade}th-grade counseling program begins in early September. However, if ${first} enrolls by ${expiryText}, ` +
    `we will start working with ${p.obj} immediately at the ${grade}th-grade package price, with no additional charge for the ` +
    `time between now and the official September start. This means ${first} gets, at no extra cost:\n` +
    ` - Full counseling support from now through early September (${durationText} of active advising, check-ins, and planning)\n` +
    `Combined, this can add up to ${bonusText} in additional service (when paired with ${topLabel}).`
  )
}

// "Comprehensive, VIP and UVIP" / "Comprehensive and VIP" / "VIP".
export function listJoin(items) {
  if (items.length <= 1) return items[0] || ''
  return `${items.slice(0, -1).join(', ')} and ${items[items.length - 1]}`
}

// Small numbers read as words in Ryan's prose ("Below are three options").
export function countWord(n) {
  return ['zero', 'one', 'two', 'three'][n] || String(n)
}

// Heading for the pay-in-full block, naming only the presented tiers that are
// actually eligible. Callers omit the block entirely when the list is empty.
export function payInFullHeading(labels) {
  return labels.length ? `Pay-in-full incentive (${listJoin(labels)})` : 'Pay-in-full incentive'
}
...
export const SEASON_ACCOMPLISH = {
  ...
  '10': {
    ...
    fall:
      ...
      '- Take on tangible leadership, ownership, and/or responsibility\n' +
...
export const PACKAGE_INCLUDED = {
  comprehensive:
    ...
    ' - Parent Communication: Email + 1 meeting per semester\n' +
  ...
  // Sourced from the live UVIP page copy. Every line is a claim the page
  // already makes; nothing here may be added to or embellished.
  uvip:
    ' - Everything in VIP, delivered as a private partnership guided by both members of our core leadership\n' +
    ' - All-inclusive and unmetered: every service we offer is included, and nothing is sold as an add-on\n' +
    ' - Director Ryan reviews every application in its entirety before submission\n' +
    ' - Reserved for 10 families per year',
}

// "Best for:" line per package (Output B26/B37/B48). VIP is the default return.
export function bestFor(pkg, { first }) {
  if (pkg === 'uvip')
    return ' - Families who want a private partnership with our leadership team, with every service included and no add-ons to manage.'
...
export const STATIC = {
  ...
  // Flat $1,000 at every price — Claude_Services.md §7 (2026-27 refresh · Aaron
  // 2026-08-31, D13 RESOLVED), matching ap-checkout /terms §6. The retired
  // wording split the incentive at $25,000, a milestone the 2026-27 ladder
  // leaves below every tier, so the $500 half could never fire and the sentence
  // put a number in front of a family that no longer existed.
  // The LEGACY (2025-26) wording that this replaced is not lost: a proposal on
  // the frozen legacy card still owes those families the tiered sentence they
  // were shown, so it lives on that preset as `payInFullBody` in
  // lib/pricingSchema.js. This constant is the CURRENT card's sentence and the
  // default for every proposal that does not carry a preset of its own.
  payInFullBody: 'If paid in full upfront, A|P may apply a one-time incentive of $1,000.',
