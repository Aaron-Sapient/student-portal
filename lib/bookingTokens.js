...
import { readMode, logShadow } from './readFlags';
...
// ── Read side (booking_tokens domain flag; see lib/readFlags.js) ───────────
// Sheets stays the WRITE authority regardless of this flag — the mirror above
// is best-effort and this reader only ever substitutes where the value is READ
// from. A missing mirror row means "no token" — mirrorBookingToken() DELETEs the
// row on an empty-string write (ART clear, and any other instructor cleared to
// ''), so no-row and a blank Master cell are the same state by construction.
async function getBookingTokenFromSupabase(studentSheetId, slug) {
  const sb = getSupabaseClient();
...
function diffBookingToken(sheetVal, supaVal) {
  const a = String(sheetVal ?? '');
  const b = String(supaVal ?? '');
  return a === b ? [] : [`sheets=${JSON.stringify(a)} supa=${JSON.stringify(b)}`];
}

// Reads one instructor's token value (AZ/BB/BD) per the `booking_tokens` read
// flag. `sheetVal` is the value the caller already has from its own Master-row
// batchGet — required both as the shadow-mode authoritative answer and as the
// on-mode fallback (so a Supabase error never changes booking behavior).
//   off (default, unset ⇒ off) → sheetVal, byte-identical to today.
//   shadow → also reads Supabase, logs any diff via logShadow, still returns
//            sheetVal — never changes what a caller sees.
//   on     → Supabase-first; on ANY read error (or no studentSheetId to key
//            on), falls back to sheetVal rather than failing the booking check.
export async function getBookingToken(studentSheetId, slug, sheetVal) {
  const mode = readMode('booking_tokens');
  if (mode === 'off' || !studentSheetId) return sheetVal;

  if (mode === 'on') {
    try {
      return await getBookingTokenFromSupabase(studentSheetId, slug);
    } catch (e) {
      console.warn(`[booking_tokens:supabase] getBookingToken fell back to Sheets: ${e?.message}`);
      return sheetVal;
    }
  }

  // shadow
  const supaVal = await getBookingTokenFromSupabase(studentSheetId, slug).catch((e) => {
    console.warn(`[shadow:booking_tokens] ${studentSheetId}/${slug} read threw: ${e?.message}`);
    return undefined;
  });
  if (supaVal !== undefined) {
    logShadow('booking_tokens', `${studentSheetId}:${slug}`, diffBookingToken(sheetVal, supaVal));
  }
  return sheetVal;
}
