...
// save time) with created_at as the reference date, so both the price inputs
// and the seasonal parts (late-start window, early-start bonus) reproduce the
// numbers the family actually saw — a contract must not move when the pricing
// dashboard does (adversarial-review finding, 2026-08-11). A legacy row with
// no snapshot re-derives against the caller's current config, and
// `configSource` says which happened so a consumer can tell a frozen figure
// from a best-effort one. email_html stays the only faithful record of the
// sent ARTIFACT; this is the faithful record of the COMMITMENT.
export function buildContract(quoteRow, config) {
  ...
  const quote = computeQuote(selection, effectiveConfig, quoteRow?.created_at)
  ...
  return {
    ...
    quotedAt: quoteRow?.created_at ?? null,
