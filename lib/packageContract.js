...
import { ADDON_DEFS, DEFAULT_PRESET, PACKAGE_LABELS, mergeConfig, offeredOn, paymentTermsFor, presetFor } from './pricingSchema'
...
export function buildContract(quoteRow, config) {
  ...
  const preset = presetFor(selection.pricingPreset)
  const offered = offeredOn(preset, selection.selectedPackages)
  ...
  for (const pkg of offered) {
    ...
    tiers[pkg] = {
      ...
      total: P.total,
      base: P.base,
      /* THE ARITHMETIC, not just the answer (2026-09-08). Added for the lead
         page, which shows a family how a total is built rather than asserting
         it: base + add-ons = subtotal, less the stacked discount, equals total.
         Every value is read off the same computeQuote result the total came
         from, so a page cannot show working that fails to reach its own total.
         ADDITIVE: no existing consumer reads these, and none of the fields
         above changed, so provisioning and the contract receipt are untouched. */
      addOnTotal: P.addOnTotal,
      subtotal: P.subtotal,
      totalDiscount: P.totalDiscount,
      totalDiscountPct: P.totalDiscountPct,
      bonusTotal: P.bonusTotal,
      /* Prose, already written for whichever card this quote is frozen on: the
         legacy preset keys terms to the TIER, the current card derives them
         from the TOTAL, and paymentTermsFor is the one place that knows which. */
      paymentTerms: paymentTermsFor(preset, P.total, pkg),
  ...
  return {
    ...
    // Not just snapshot-vs-current any more: a snapshot-less row now falls back
    // to ITS OWN preset's card, so labelling that "current" would report legacy
    // figures under the name of today's ladder — and this field's whole job is
    // telling a frozen figure from a best-effort one.
    configSource: snap ? 'snapshot' : preset.key === DEFAULT_PRESET ? 'current' : `preset:${preset.key}`,
    student: {
