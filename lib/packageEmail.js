...
function discountSummary(state, config, quote) {
  ...
  // The ceiling trim depends on each package's own add-on count, so an effective
  // rate can differ across the three. A single "(5%)" heading above three
  // different rates would be a false statement, so the rate is printed only when
  // all three agree and is dropped otherwise.
  const rateLabel = (field) => {
    const vals = PACKAGES.map((p) => Math.round(pk[p][field] * 10) / 10)
    return vals.every((v) => v === vals[0]) ? ` (${vals[0]}%)` : ''
  }
  if (disc.referral) lines.push(` - Referral Discount${rateLabel('referralPctEffective')}: ${triplet((p) => money(pk[p].referralDiscount))}`)
  if (disc.sibling) lines.push(` - Sibling Discount${rateLabel('siblingPctEffective')}: ${triplet((p) => money(pk[p].siblingDiscount))}`)
  const custom = Math.max(0, Number(disc.custom) || 0)
  if (custom > 0) lines.push(` - Custom Discount${rateLabel('customPctEffective')}: ${triplet((p) => money(pk[p].customDiscount))}`)
