...
import { CLOSED_PACKAGES, DEFAULT_PRICING, PACKAGE_LABELS, normalizeSelectedPackages, paymentTermsForTotal } from './pricingSchema'
...
export function buildEmail(state, config, refISO) {
  ...
  // The tiers Ryan is presenting, cheapest first.
  ...
  for (const pkg of offered) {
    ...
    p(`     - ${PACKAGE_LABELS[pkg]} (${money(total)}): ${paymentTermsForTotal(total, pkg)}`)
  }
  sp()
  ...
  const payInFull = offered.filter((pkg) => eligible.includes(pkg) && !CLOSED_PACKAGES.includes(pkg))
  if (payInFull.length) {
