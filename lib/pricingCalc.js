...
//
// ONE DELIBERATE DIVERGENCE FROM THE SHEET (2026-08-06): the stacking discounts
// (per-add-on + referral + sibling + custom) are capped at config.discounts
// .maxStackPct, default 25%. The sheet has no cap in any cell — verified by
// reading Menu!P17 and Menu!P21 directly — so an uncapped stack could reach 36%
// before any custom discount. Ryan requested the ceiling and it applies to the
// stack only, NOT to late-start. Until the sheet carries the same cap, the two
// generators disagree above 25%; see the Backend!B22:B24 note in packageEmail.js.
...
export function computeQuote(state, config, refISO) {
  ...
  // fraction, e.g. 0.1. Clamped to [0,1]: the builder's max={100} is an HTML
  // hint only, so a typed or programmatic 200 would otherwise produce a
  // negative total.
  const custom = Math.min(1, Math.max(0, Number(disc.custom) || 0))
  ...
  for (const pkg of PACKAGES) {
    ...
    // Requested stack, as percentages. Late-start is deliberately absent: it is
    // not a stacking discount (see maxStackPct in pricingSchema).
    let addOnPct = categories * config.discounts.perAddOnPct
    let referralPct = disc.referral ? config.discounts.referralPct : 0
    let siblingPct = disc.sibling ? config.discounts.siblingPct : 0
    let customPctEff = custom * 100
    const requestedStackPct = addOnPct + referralPct + siblingPct + customPctEff

    // Trim to the ceiling in PRIORITY ORDER rather than proportionally. The
    // per-add-on discount is the only component the proposal email prints
    // without an advertised rate, so it can shrink without making the email
    // contradict itself; referral and sibling are promised at a flat 5% and are
    // trimmed last. A trim that reaches them means the config itself is
    // over-subscribed, and capApplied surfaces that to Ryan in the builder.
    const ceilingPct = Number(config.discounts.maxStackPct)
    const hasCeiling = Number.isFinite(ceilingPct) && ceilingPct >= 0
    const capApplied = hasCeiling && requestedStackPct > ceilingPct
    if (capApplied) {
      let over = requestedStackPct - ceilingPct
      const fromAddOn = Math.min(addOnPct, over)
      addOnPct -= fromAddOn
      over -= fromAddOn
      const fromCustom = Math.min(customPctEff, over)
      customPctEff -= fromCustom
      over -= fromCustom
      if (over > 0 && referralPct + siblingPct > 0) {
        const keep = Math.max(0, referralPct + siblingPct - over) / (referralPct + siblingPct)
        referralPct *= keep
        siblingPct *= keep
      }
    }
    const stackPct = addOnPct + referralPct + siblingPct + customPctEff

    const perAddOnDiscount = (addOnPct / 100) * afterLS
    const referralDiscount = (referralPct / 100) * afterLS
    const siblingDiscount = (siblingPct / 100) * afterLS
    const customDiscount = (customPctEff / 100) * afterLS
    ...
    packages[pkg] = {
      ...
      afterLS,
      categories,
      // Effective rates AFTER any ceiling trim — the email must label lines with
      // these, never with the configured rate, or it prints "(5%)" beside a
      // delivered 3.5%.
      perAddOnPctEffective: addOnPct,
      referralPctEffective: referralPct,
      siblingPctEffective: siblingPct,
      customPctEffective: customPctEff,
      requestedStackPct: Math.round(requestedStackPct * 100) / 100,
      stackPct: Math.round(stackPct * 100) / 100,
      capApplied,
  ...
  return {
    ...
    maxStackPct: Number(config.discounts.maxStackPct),
