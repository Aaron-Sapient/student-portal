...
import { sheetSafe } from './sheetSafe.js';
...
`.trim();
    ...
    // Capture the timestamp + trimmed name ONCE so col A and the Supabase mirror
    // are byte-identical (the reconcile backfill re-derives report_at from col A).
    // Native Date is CORRECT here and deliberately kept: this is a zone-independent
    // INSTANT serialized as UTC ISO, not calendar-date logic, so there is no off-by-one
    // to have. The project's "avoid native Date()" rule targets date reasoning.
    ...
    try {
      appendRes = await sheets.spreadsheets.values.append({
        ...
        requestBody: {
          values: [[
            reportAt,                                // A: Date (server-generated)
            sheetSafe(reportStudentName),            // B: Student (trimmed — the upload matches master col A exactly)
            sheetSafe(sections.onTarget),            // C: On Target        ) model-authored: a report
            sheetSafe(sections.needsAttention),      // D: Needs Attention  ) section opening with '='
            sheetSafe(sections.strategy),            // E: Strategy & Recommendations ) would otherwise
            sheetSafe(sections.parentRequests),      // F: Parent Requests  ) become a live formula
            false,                                   // G: Status (unchecked — boolean keeps its checkbox typing)
          ]],
        },
      });
    } catch (sheetErr) {
    ...
    // Best-effort dual-write to the Supabase written_reports mirror (Bucket-A
    // cutover). NON-authoritative: keyed on sheet_row parsed from the append's
    // updatedRange, so it matches both the reconcile backfill and the developer
    // route's rowIndex. A mirror failure must never affect report generation, and
    // no read flag is flipped — this only keeps the mirror fresh.
    if (appendRes && studentSheetId) {
      try {
        const updatedRange = appendRes.data?.updates?.updatedRange || '';
        const rowMatch = updatedRange.match(/![A-Z]+(\d+)/);
        const sheetRow = rowMatch ? Number(rowMatch[1]) : null;
        if (sheetRow) {
          ...
          const { error } = await getSupabaseClient()
            ...
            }, { onConflict: 'sheet_row', ignoreDuplicates: false });
          if (error) console.warn('[dual-write:written_reports] insert failed:', error.message);
        } else {
          console.warn('[dual-write:written_reports] could not parse sheet_row from:', updatedRange);
        }
      } catch (e) {
        console.warn('[dual-write:written_reports] skipped:', e?.message || e);
      }
    }
