...
import { getSupabaseClient, STUDENT_PROFILES, WRITTEN_REPORTS } from './supabase.js';
import { recentCheckins } from './checkinRecords.js';

const REPORT_ZONE = 'America/Los_Angeles';

// A Sheets date cell reads back either as a serial (days since 1899-12-30) under
// UNFORMATTED_VALUE or as a string. Both denote a CALENDAR DATE, not an instant — and the
// distinction is the whole bug. A serial must be rebuilt in UTC and only THEN re-anchored
// in the portal zone: rebuilding it directly in America/Los_Angeles lands on 17:00 the
// previous day and formats to the wrong date. That is the exact defect fixed in f29e12d,
// and it is why "always force America/Los_Angeles" is the wrong instruction at the PARSE
// step for a zoneless value. Matches the idiom in lib/meetings.js, lib/projects.js,
// lib/seniorsCore.js, app/api/home-data and scripts/mirrorComps.cjs.
function sheetCellToDate(raw) {
  ...
  const dt = DateTime.fromISO(String(raw).trim(), { zone: REPORT_ZONE });
  return dt.isValid ? dt : null;
}

// No Master sheet id, tab names or sheetSafe: nothing in this module writes a
// spreadsheet any more. The only Google calls left are the two student-sheet READS
// inside fetchStudentData (🏆 Comps & Projects and the 🎓 Transcript summary block),
// which belong to the comps and transcript lanes.
...
function getServiceAuth() {
  return new google.auth.GoogleAuth({
    ...
    scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'], // read-only: the writes are gone
  });
}
...
// ── Overview profile (name / grade / major / SAT / #APs) for the Claude prompt ──
// Was 🔎 Overview!B2:D20 on the student's sheet behind READ_SUPABASE_OVERVIEW_PROFILE.
// The mirror (mirrorStudentHub.cjs) stores the same UNFORMATTED values as text, so
// the shape reconstructs identically. Flag and Sheets reader both deleted.
async function readOverviewProfile(studentSheetId, studentName) {
  const { data, error } = await getSupabaseClient()
    .from(STUDENT_PROFILES)
    .select('display_name, current_year, major, sat, num_aps')
    .eq('student_sheet_id', studentSheetId)
    .maybeSingle();
  if (error) throw new Error(`overview profile read failed: ${error.message}`);
  return {
...
async function fetchStudentData(sheets, studentSheetId, studentName) {
  const [overview, projectsRes, transcriptRes] = await Promise.all([
    readOverviewProfile(studentSheetId, studentName),
  ...
  const startOfThisYear = DateTime.now().setZone(REPORT_ZONE).startOf('year');
  ...
  const activeProjects = projectRows
    .filter(r => {
      ...
      const endDate = sheetCellToDate(rawDate);
      return !endDate || endDate >= startOfThisYear;
    })
    .map(r => {
      const fmtDate = (raw) => {
        const d = sheetCellToDate(raw);
        return d ? d.toFormat('LLL d, yyyy') : '';
      };
      return {
        activity: r[0] || '',
...
// The last 3 check-ins as the Claude prompt's context: the newest is `current`,
// the two before it are `gradeHistory`.
//
// Was: read MASTER CheckinForm!A:L and filter rows whose col B matched the
// student's name trim-insensitively — a workaround for live spelling divergence
// (the comment named Aasrith's trailing space). Now keyed on student_sheet_id, so
// the workaround and the class of bug it patched are both gone.
//
// This is a read-after-own-write: submitUpdateForm inserts the check-in and then
// calls triggerReportGeneration in the same request. Under the old dual-write it
// had to stay on Sheets because the Supabase mirror was reconcile-lagged and would
// miss the just-appended row. Under single-write it reads the row this request
// just inserted, from the same store — correct by construction.
async function fetchCheckinHistory(studentSheetId) {
  const rows = await recentCheckins(studentSheetId, 'ryan', 3);
  if (!rows.length) return null;

  const [latest, ...older] = rows; // newest first
  const p = latest.payload || {};
  return {
    current: {
      grades: p.grades || '',
      testsDeadlines: p.tests_and_deadlines || '',
      taskUpdates: p.task_updates || '',
      concernCategory: p.concern_category || '',
      concernText: p.concern_text || '',
      selfRating: p.self_rating || '',
    },
    // oldest -> newest, matching the old .slice(-3) ordering
    gradeHistory: older
      .slice()
      .reverse()
      .map((r) => ({ timestamp: r.submitted_at || '', grades: r.payload?.grades || '' })),
  };
}

// The student's gender, for the report's pronouns. Was Master A:AX (col AX = 49)
// matched on col A. `students.gender` is the same value, keyed.
async function fetchGender(studentSheetId) {
  const { data, error } = await getSupabaseClient()
    .from('students')
    .select('gender')
    .eq('student_sheet_id', studentSheetId)
    .maybeSingle();
  if (error) throw new Error(`gender read failed: ${error.message}`);
  return data?.gender || '';
}
...
export async function triggerReportGeneration(studentName, studentSheetId) {
  try {
    ...
    const [studentData, checkinHistory, gender] = await Promise.all([
      ...
      fetchCheckinHistory(studentSheetId),
      fetchGender(studentSheetId),
    ]);
    ...
    const gradeHistoryText = checkinHistory.gradeHistory.length
      ? checkinHistory.gradeHistory.map((h, i) =>
          // h.timestamp is a real INSTANT (a check-in submission time), so it is parsed as
          // one and rendered in the portal zone. Bare toLocaleDateString() was doubly
          // non-deterministic here: it took both the zone AND the locale from the server.
          `Submission ${i + 1} (${
            h.timestamp
              ? DateTime.fromISO(String(h.timestamp)).setZone(REPORT_ZONE).toFormat('L/d/yyyy')
              : 'unknown'
          }): ${h.grades}`
        ).join('\n')
      : 'No previous grade submissions on record.';

    // "Today" as the student experiences it, not as Vercel's UTC clock sees it — on a
    // report generated after 5pm Pacific those are different days.
    const today = DateTime.now().setZone(REPORT_ZONE).toFormat('cccc, LLLL d, yyyy');
...
`.trim();
    ...
    // ── Write the report. ONE store: `written_reports`. ─────────────────────
    // Was an append to MASTER WrittenReports!A:G, with `sheet_row` scraped out of
    // the append response's updatedRange and used as the join key for a best-effort
    // Supabase mirror. Sheets was authoritative; Postgres was the copy.
    //
    // Now inverted, and the scraped row number is gone with it. The row's own uuid
    // is the key the developer panel edits and uploads against, so a report is no
    // longer addressed by its position in a spreadsheet. `sheet_row` stays NULL on
    // new rows (the column is nullable and still carries the 60 backfilled rows).
    //
    // This THROWS on failure where the sheet append only logged. That is the point:
    // written_reports is the sole copy of the report now, and a swallowed error
    // would mean Claude generated a report that exists nowhere. sheetSafe() is gone
    // with the sheet — a leading "=" is a formula hazard, not a Postgres one.
    //
    // Native Date is CORRECT here and deliberately kept: a zone-independent INSTANT
    // serialized as UTC ISO, not calendar-date logic. The project's "avoid native
    // Date()" rule targets date reasoning.
    const reportAt = new Date().toISOString();
    const reportStudentName = String(studentData.name ?? '').trim();
    const reportDate = DateTime.fromISO(reportAt, { zone: 'utc' })
      .setZone('America/Los_Angeles').toISODate();
    const { error: reportErr } = await getSupabaseClient()
      .from(WRITTEN_REPORTS)
      .insert({
        student_sheet_id: studentSheetId,
        student_name: reportStudentName || null,
        report_at: reportAt,
        report_date: reportDate,
        on_target: sections.onTarget || null,
        needs_attention: sections.needsAttention || null,
        strategy: sections.strategy || null,
        parent_requests: sections.parentRequests || null,
        status: false,
        parent_notified: false,
      });
    if (reportErr) throw new Error(`written_reports insert failed: ${reportErr.message}`);
