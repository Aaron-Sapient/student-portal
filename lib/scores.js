...
// CURVED score instead: a perfectly average raw 50 shows as 70, and movement
// gets RPG-style harder the farther a score sits from that pivot (tanh tails
// in both directions — each extra display point costs more raw points).
...
// saturates (tanh), but climbing above 70 is a slow cubic grind — the model
// already hands out raw 70s/80s to merely-good students, so the display must
// not stretch them further. 90+ display = raw mid-90s = genuinely elite.
const RAW_CENTER = 50 // the model's "perfectly average" raw score
const PIVOT = 70 // what that average student sees
const CENTER_SLOPE = 1.2 // below-pivot: display points per raw point at the pivot
const UP_GAMMA = 3 // above-pivot exponent (higher = flatter just above 70)
...
// Reference (10th/11th, no lift): raw 0→21 · 20→37 · 30→47 · 40→58 · 47→66 ·
//            50→70 · 60→70 · 71→72 · 80→76 · 85→80 · 90→85 · 95→92 · 100→100
