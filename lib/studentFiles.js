...
// This proxy exists to RENDER counselor-authored HTML on the portal's own origin.
// That is also its danger: without a sandbox, a document served here runs with the
// viewer's Clerk session and can call every API they're authorized for. The sandbox
// directive drops the document into an opaque origin — no cookies, no same-origin
// fetch, no portal localStorage — which is the whole mitigation. The three allow-*
// tokens are the minimum that keeps real reports working, and NONE of them is
// `allow-same-origin`, which is the one that would give the origin back:
...
// Refuse to buffer something enormous: the whole body lands in the function heap
// twice (arraybuffer + Buffer), and Vercel caps the response anyway. Student
// reports are tens of KB; this only ever trips on a mistargeted id.
const MAX_PROXY_BYTES = 25 * 1024 * 1024

// Fetch a Drive file's bytes and wrap them in a web Response carrying the file's
// real Content-Type, so uploaded HTML renders instead of showing as raw source.
// `file` is the already-resolved {id,name,mimeType,size} — resolving it is where
// the access decision lives, and this helper deliberately makes none.
async function driveMediaResponse(drive, file) {
  ...
  try {
    ...
    return new Response(buffer, {
      headers: {
        ...
        'Content-Security-Policy': RENDER_SANDBOX,
...
export async function streamDriveFileFromFolder(folderId, fileId) {
  ...
  return driveMediaResponse(drive, file)
}

// Stream ANY Drive file the service account can read, with NO folder gate. The
// caller is the gate: this is only ever reached after the route has confirmed an
// ADMIN_EMAILS session (Aaron/Ryan), who already hold full human access to the
// firm's Drive — so the folder scoping that protects student sessions buys
// nothing here and only breaks staff opening the same link they sent a family.
export async function streamDriveFileAsAdmin(fileId) {
  const drive = getGoogleDriveClient()
  let file
  try {
  ...
  if (!file?.id || file.trashed) return new Response('Not found', { status: 404 })
  // Native Docs/Sheets/Slides have no downloadable bytes — alt:'media' 403s on
  // them, which would surface as a bare 502. Say what actually happened instead.
  if (String(file.mimeType || '').startsWith('application/vnd.google-apps.')) {
    return new Response('That is a native Google file — open it in Drive.', {
      status: 415,
      headers: { 'Cache-Control': 'private, no-store' },
    })
  }
