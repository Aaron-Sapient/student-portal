...
import { getStudentProfile, studentDisplay } from '@/lib/identity'
...
export async function resolveCheckinStudent() {
  ...
  //
  // ⚠ NO `class` is passed. studentDisplay's last-resort degrade is
  // gradeLabelFromClass(), which DERIVES a grade from class-year arithmetic — and
  // this value is not for display, it picks the 🎓 Transcript row block the
  // submission's grades are WRITTEN into. Before the Supabase move, a failed
  // Overview read left gradeYear null and buildGradeWriteData returned [], so the
  // grades were simply skipped; letting a computed year through would instead write
  // real grades into a guessed block. A roster grade would be safe to pass here, but
  // this function still resolves identity from the Master row (Package D retires
  // that read), and the Master carries no grade column — so on a missing profile the
  // value stays null and the write is skipped, exactly as before.
  ...
  return {
    email,
    // 1-based sheet row, matching the old getUpdateFormData `rows.indexOf(...) + 1`.
    // Still read from the Master because the POST writes AY/BA${rowIndex} —
    // Package D's rowIndex elimination retires this read with those writes.
