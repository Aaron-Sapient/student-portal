...
// `excludeEventId` — DISPLAY ONLY, for the reschedule slot list. A meeting being moved
// must not trip the 1/week cap against itself, or every day in its own week reads
// "week-booked" and the student sees an empty calendar and concludes the instructor has
// no availability (that is exactly what happened to a student on 2026-08-24).
//
// It is NOT how the cap is enforced on the write path, and must never become that: the
// cap also lives in Postgres as a partial unique index (pmb_one_active_per_week), which
// an in-memory filter cannot speak to. The authority for a move is
// projectRescheduleConflict, which excludes the row by id and runs before the calendar is
// touched. Pass an id here only after it has been ownership-verified (fetchOwnedEvent /
// resolveRescheduleTarget) — unverified, it is a "don't count my meeting" flag.
export async function loadProjectBookingsForPlan(planId, now, excludeEventId = null) {
  if (!planId) return [];
  const sb = getSupabaseClient();
  const fromWeek = weekStartISO((now || DateTime.now()).setZone(ZONE));
  const { data } = await sb
    .from(PROJECT_MEETING_BOOKINGS)
    .select('id, week_start, meeting_date, status, calendar_event_id')
    .eq('plan_id', planId)
    .eq('status', 'active')
    .gte('week_start', fromWeek);
  const rows = data || [];
  return excludeEventId ? rows.filter((r) => r.calendar_event_id !== excludeEventId) : rows;
}
...
// The ACTIVE ledger row an event owns, or null. Distinct from projectRescheduleConflict,
// which answers "would this move collide" and correctly returns null both when there is no
// conflict AND when the event has no row at all. Callers that are about to MOVE a booking
// need those two cases separated: an event with project extendedProperties but no live row
// is an orphan (bookMeeting leaves one when its post-insert rollback delete fails), and
// moving it would update zero rows while reporting success.
export async function loadActiveProjectBookingByEventId(eventId) {
  if (!eventId) return null;
  const sb = getSupabaseClient();
  const { data, error } = await sb
    .from(PROJECT_MEETING_BOOKINGS)
    .select('id, plan_id, week_start, meeting_date, minutes')
    .eq('calendar_event_id', eventId)
    .eq('status', 'active')
    .maybeSingle();
  if (error) throw error; // fail closed — see projectRescheduleConflict
  return data || null;
}
...
export async function projectRescheduleConflict(eventId, newDt) {
  ...
  const { data: row, error: rowErr } = await sb
    ...
    .eq('status', 'active')
    .maybeSingle();
  // FAIL CLOSED. This is the only cap authority on the student reschedule path, so a
  // swallowed error here reads as "no conflict" and lets a second active booking into a
  // week. Note maybeSingle() also errors when MORE than one row matches — nothing
  // constrains calendar_event_id to be unique (pmb_event_idx is a plain index) — and
  // that case is exactly when the caller must not proceed.
