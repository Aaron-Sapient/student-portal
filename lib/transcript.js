...
// UNFORMATTED_VALUE matches all three production reads (home-data / parent / coach)
// EXACTLY — a percent grade is the serial 0.85, not "85%", and looksLikeGrade keys
// on that representation, so the render option is load-bearing for byte-identity.
// Does NOT catch: each call site keeps its own read-failure policy (coach fails
// OPEN → true; home-data/parent fall through to hasRecentGrades([])).
