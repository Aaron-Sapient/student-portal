...
// Source-agnostic core of the recency gate. `probe(grade, sem)` answers "is there
// a recorded grade in this (gradeLevel, semester) slot?" — the ONLY thing that
// differs between the Sheets transcript grid and the Supabase transcript_entries
// mirror. Everything else (term/grace/candidate logic) is shared, so the two
// sources are byte-identical by construction (the lib/scores.js curveEntry trick,
// applied to the gate). Returns { enough, reason, grade }.
function hasRecentGradesWith(probe, classStr, nowLA) {
  ...
  if (candidates.some((s) => probe(s.grade, s.sem))) {
...
// Sheets path: the original signature, unchanged behavior — feeds the grid probe
// into the shared core. (Keeps every existing caller + the NAS cron byte-identical.)
// transcript = 🎓 Transcript!A1:V40 values (2-D array, A1-origin).
function hasRecentGrades(transcript, classStr, nowLA) {
  return hasRecentGradesWith((grade, sem) => slotHasGrade(transcript, grade, sem), classStr, nowLA);
}
