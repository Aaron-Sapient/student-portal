...
// Student file delivery (consultant design 2026-08-27, supabase/documents.sql):
// one `documents` row per file per student; the student's working copy in
// `document_edits`; pdf / >1 MB bodies in the private Storage bucket.
export const DOCUMENTS = 'documents'
export const DOCUMENT_EDITS = 'document_edits'
...
// Standard (Ryan/Aaron) + ART booking RECORD — the track that had no Postgres ledger
// until 2026-08-27 (SUMMER-EXIT.md W3: "a Google Calendar event ... IS the entire
// state"). Calendar keeps the event; this row is the truth. Written FIRST by
// bookMeeting, then the event is created and attached. See supabase/bookings.sql and
// lib/bookings.js (which also unions all three ledgers for the readers).
...
// isn't immediately stale. THIS IS AN APP READER'S TABLE now (the old "no app
// reader exists yet — validateBooking's cap check stays on Sheets" note went stale
// with the zero-Google ruling, 2026-08-27): app/api/validateBooking reads it
// directly, no flag, and a read ERROR there fails closed. Derive the current reader
// set rather than trusting this line — `command grep -rn MEETING_CAP_SUMMARY app lib
// scripts`. See _notes/cutover-execution-plan.md.
export const MEETING_CAP_SUMMARY = 'meeting_cap_summary'

// Autonomous meeting-compliance outreach (senior essay track). OUTREACH_LOG is the
// per-week send/dry-run ledger whose unique (student_sheet_id, week_start, kind) key
// is the atomic dedup; OUTREACH_SUPPRESSIONS silences the nudge for a student during
// a known no-meeting stretch. Service-role only. See supabase/outreach_log.sql +
// supabase/outreach_suppressions.sql and lib/complianceOutreach.js.
export const OUTREACH_LOG = 'outreach_log';
export const OUTREACH_SUPPRESSIONS = 'outreach_suppressions';
...
// Per-lead post-consult pages (/next/<slug>). One row per family: slug is the
// address, `data` is the whole lead JSON verbatim. Read per request by
// app/next/[slug]/leads.js so a new family needs an INSERT, not a rebuild.
// Service-role only (RLS enabled, no policies) — the rows carry a minor's name,
// a parent's email and phone, and that family's quoted prices, and the
// student-hubs publishable key is student-visible. See supabase/lead_pages.sql.
export const LEAD_PAGES = 'lead_pages'
