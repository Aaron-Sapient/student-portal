...
/* The short code for a zone ("SGT", "PDT", "IST"), DERIVED from the zone name
   rather than typed beside it, so a lead in a new city cannot be given the
   wrong one by a data-entry slip.

   It takes a sweep of locales because no single one knows every abbreviation:
   en-US calls Singapore "GMT+8" and Los Angeles "PDT", en-SG has "SGT", en-IN
   has "IST". The first ALPHABETIC answer wins; a zone no locale here has a
   letter code for keeps the numeric "GMT+8" form, which is still true and
   still short. Never a lookup table: a table is a second place for the zone to
   be wrong. */
const ABBR_LOCALES = ['en-US', 'en-SG', 'en-IN', 'en-GB', 'en-AU'];

export function zoneAbbr(timezone, at) {
  let numeric = null;
  for (const locale of ABBR_LOCALES) {
    try {
      ...
      /* An unknown zone name: try the next locale, then give up quietly. */
    }
  }
  return numeric;
}
...
/* One slot, said twice. The family's clock leads because it is the one they
   ...
   The CHIP shows only the family's side, as "7:00 am SGT" (2026-09-04, Aaron).
   Ryan's time under all eight chips was the same fact eight times, and it cost
   every chip a second line to say something the family is not choosing between.
   Both clocks survive where they are load-bearing: on the confirm sentence, at
   the moment the family commits, and in every chip's aria-label. */
export function describeSlot(slot, timezone, zoneLabel) {
  ...
  return {
    ...
    family: {
      time: t(fam),
      day: fam.toFormat('cccc'),
      date: fam.toISODate(),
      zone: zoneLabel,
      abbr: zoneAbbr(timezone, fam.toJSDate()),
    },
