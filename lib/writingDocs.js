...
export async function ensureDocuments(supabase, studentSheetId, student) {
  ...
  if (missing.length) {
    ...
    for (const d of created || []) {
      byType[d.doc_type] = d
      // The Common App has no external source-of-truth to sync against (unlike
      // the college-list-driven PIQ/Supplement docs), so it gets one default tab
      // seeded at birth. A normal tab thereafter — freely deletable, never resurrected.
