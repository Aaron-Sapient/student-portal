...
// Week 1 of a month is, by definition (see weekOfMonth's comment, which this
// gap-fills — see the note there), THE week containing that month's 1st — even
// when the week's own start date falls in the PRIOR month, which is true whenever
// the month doesn't start on a Saturday (i.e. most months). A week can straddle at
// most one month boundary (months always run ≥28 days), so it's either the week
// that itself STARTS on a 1st, or the week whose END rolls into the next month
// (whose 1st is necessarily that end date's month) — never both, never neither
// unless the week sits entirely inside one month.
export function weekOneMonthKey(dt) {
  const weekStart = startOfSaturdayWeek(dt);
  if (weekStart.day === 1) return weekStart.toFormat('yyyy-LL');
  const weekEnd = weekStart.plus({ days: 6 });
  return weekEnd.month !== weekStart.month ? weekEnd.toFormat('yyyy-LL') : null;
}

// The 'yyyy-LL' month that week `w` is senior's phase-th week FOR, or null if it
// isn't — NOT necessarily `w`'s own month: a phase-1 week can start in the prior
// month (see weekOneMonthKey), so its "own" month and the month it's week-1 OF can
// differ. Phases 2-4 never straddle a boundary, so `w`'s own month is correct there.
function phaseWeekTargetMonth(w, phase) {
  return phase === 1 ? weekOneMonthKey(w) : weekOfMonth(w) === phase ? w.toFormat('yyyy-LL') : null;
}

// Is the week containing `dt` senior's phase-th week? Single source of truth for
// the phase-1 boundary case (weekOneMonthKey) so no caller — including a future
// one — has to special-case it itself the way weekOfMonth alone cannot.
function isPhaseWeek(dt, phase) {
  return phaseWeekTargetMonth(startOfSaturdayWeek(dt), phase) != null;
}

// The grant's phase week: whichever of its two candidate Saturday-weeks
// (week_start, week_start+1wk) is senior's phase week, paired with the month it's
// week-1 OF (see phaseWeekTargetMonth) — or null if neither week is.
function matchingPhaseWeek(senior, grant) {
  if (!grant) return null;
  const w1 = DateTime.fromISO(grant.week_start, { zone: ZONE }); // a Saturday
  if (!w1.isValid) return null;
  const w2 = w1.plus({ weeks: 1 });
  for (const start of [w1, w2]) {
    const monthKey = phaseWeekTargetMonth(start, senior.phase);
    if (monthKey) return { start, monthKey };
  }
  return null;
}
...
//
// `cross_month_override` is the CARRIED-OVER case: a check-in that supersedes a
// still-live grant which still owed an unbooked cross-meeting stamps that month
// here, so the entitlement survives the supersede instead of silently vanishing
// (seniors.createCheckinGrant → pendingCrossMonth; rationale and the measured
// incident are in supabase/senior_cross_month_override.sql). The grant's own phase
// week always wins, so an override never adds a SECOND month.
//
// It is not self-limiting, though, and the two guards that bound it live elsewhere:
// carriedCrossMonth() refuses to stamp a month that is already past or whose cross
// was already taken (so a stale month can't ride forward on later check-ins), and
// weeklyVerdict() withholds CAPACITY RESERVATION from an override-carried cross.
// Note crossMeetingDone() matches bookings on this monthKey, not on the meeting's
// own month — a grant window straddles a month boundary, so a cross booked on the
// far side does not read as done here. That is why pendingCrossMonth ALSO checks the
// outgoing grant's own secondary bookings before carrying anything.
export function phaseWeekMonthKey(senior, grant) {
  return matchingPhaseWeek(senior, grant)?.monthKey ?? grant?.cross_month_override ?? null;
}

// The phase week's concrete { start, end } ISO bounds for this grant — the
// boundary-proof source the getMonthAvailability gold "phase week" highlight
// derives from, so display can never diverge from what phaseWeekMonthKey /
// canBookOnDate actually authorize (a day-by-day `weekOfMonth(d) === phase` scan
// confined to the viewed month would miss the lead days of a phase-1 week that
// starts in the prior month).
// DELIBERATELY not override-aware, and this is not an oversight to "fix": a
// cross_month_override exists precisely BECAUSE the grant's window contains no phase
// week, so there are no week bounds to return. Such a cross is bookable across the
// whole grant window, not confined to a week, so the gold highlight is correctly
// absent while the cross card still shows. Teaching this the override would invent a
// week the authorization doesn't recognise — the exact divergence the note above warns
// against, in the opposite direction.
export function phaseWeekBounds(senior, grant) {
  const m = matchingPhaseWeek(senior, grant);
  return m ? { start: m.start.toISODate(), end: m.start.plus({ days: 6 }).toISODate() } : null;
}
...
// The `cross_month_override` to stamp on a grant that is about to SUPERSEDE `outgoing`
// — i.e. the cross-meeting entitlement that would otherwise be destroyed. Returns null
// (the normal case) when nothing is owed. PURE: the IO layer (seniors.pendingCrossMonth)
// supplies `outgoing` (null unless a grant is still LIVE — a naturally-lapsed grant is
// use-it-or-lose-it and must never be carried) and the months in which the student
// already has an active secondary-teacher booking.
export function carriedCrossMonth(senior, outgoing, incoming, bookedSecondaryMonths = [], now) {
  if (!outgoing) return null;
  // Reads the outgoing grant's own override too, so the entitlement survives a chain
  // of weekly check-ins, not just the first supersede.
  const month = phaseWeekMonthKey(senior, outgoing);
  if (!month) return null;
  if (bookedSecondaryMonths.includes(month)) return null; // already taken this month
  // EXPIRY. Without this the month string rides forward forever: every later check-in
  // re-reads it off the outgoing grant and re-stamps it, so a cross the student never
  // took in August would still be "owed" in December. A cross-meeting belongs to a
  // calendar month, so it is meaningless once that month is past — anything earlier
  // than the current month is dropped here, which is what breaks the chain.
  const nowM = (now || DateTime.now()).setZone(ZONE).toFormat('yyyy-LL');
  if (month < nowM) return null;
  // If the incoming grant reaches the same month's phase week on its own, it already
  // carries it — an override would be redundant, never additive.
  return phaseWeekMonthKey(senior, incoming) === month ? null : month;
}
...
//
// ⚠ KNOWN PROXY — deliberately left in place (Aaron, 2026-08-11). Read this before
// "fixing" it, and before trusting it in a new caller.
//
// WHAT IT DOES. The second branch compares a booking's DATE-month against `monthKey`,
// the month of the entitlement the grant carries. Those are not the same thing: a
// grant window is two Saturday-weeks and a third of them straddle a month boundary
// (measured 2026-08-11: 37 of 108), so a cross-meeting for month M can legitimately be
// HELD on a date in month M-1. The date-month is a stand-in for "which entitlement did
// this booking spend", and the row cannot answer that question — `crossMeetings` is
// selected by date range and carries no grant_id (see seniors.loadSeniorBookingState).
//
// WHY IT STAYS. The month-level check is load-bearing, not an accident: two consecutive
// check-ins routinely produce two grants carrying the SAME month's cross (a real pair:
// week_start 2026-06-27 and 2026-07-04, both '2026-07'). Per-grant checking would hand
// out a second cross every time that happens. Removing the month comparison is strictly
// worse than the proxy.
//
// HOW OFTEN IT'S WRONG. 1 of 20 cross-meetings booked to date — one student booked his
// July cross on June 30. Nobody has been affected in either direction yet.
//
// BOTH FAILURE DIRECTIONS, and the quiet one is the dangerous one:
//   • false negative — a cross booked outside its entitlement's month isn't seen, so a
//     later grant carrying that month offers a second one.
//   • false positive — a cross held in month M that spent M-1's entitlement makes a
//     later grant think M's cross is done, SILENTLY costing a student a meeting they
//     were owed. No error, nothing to notice. This is the one to care about.
//
// THE FIX, IF SOMEONE TAKES IT ON. Carry `grant_id` through `crossMeetings`, resolve
// each booking's own monthKey, and compare monthKey to monthKey. That preserves every
// case the current check handles and closes the residue. Not urgent: the grant-scoped
// spent-check in seniors.pendingCrossMonth already stops cross_month_override from
// compounding it.
...
function weeklyVerdict(senior, dt, teacherSlug, mins, state) {
  ...
  // Capacity is reserved only for a cross this grant carries NATURALLY — i.e. its own
  // window contains the phase week. A CARRIED-OVER cross (cross_month_override) must
  // NOT reserve: the override rides forward on every weekly check-in until it's used,
  // so reserving a slot for it would quietly cut the student's primary allowance every
  // week for the rest of the month (a VIP to 1 meeting/week, an Essential from one
  // 30-min to one 15-min). The override still GRANTS the cross — see `carries`.
  const crossOwed = carries && !crossDone && matchingPhaseWeek(senior, grant) != null;
...
export function canBookOnDate(senior, dt, teacherSlug, mins, state) {
  ...
  // A one-off must stay genuinely ADDITIVE (see the track's own comment above): it
  // may only fill a gap the weekly grant truly has none for. When the denial is
  // 'cross-reserved', capacity DOES exist — it's held for the monthly secondary-
  // teacher meeting — so a one-off for the SAME (primary) teacher must not be
  // allowed to quietly take that slot instead of steering the student to book the
  // secondary teacher. A one-off for the secondary teacher, or any other denial
  // reason (genuinely no room left), still falls through as before.
  if (weekly.reason === 'cross-reserved' && teacherSlug === senior.primary_teacher) return weekly;
...
// NOT boundary-proof for a phase-1 check on its own: a week that starts in the
// prior month but contains the NEW month's 1st scores as the prior month's last
// week here, never as the new month's week 1. Callers checking phase eligibility
// go through isPhaseWeek/weekOneMonthKey instead, which handle that case.
...
export function assignedPlanForWeek(senior, dt) {
  ...
  const secondary = OTHER[primary];
  const rule = PACKAGE_RULES[senior.package];
  const phaseWeekNow = isPhaseWeek(dt, senior.phase);
  return { isPhaseWeek: phaseWeekNow, primarySlug: primary, secondarySlug: secondary, secondaryRequired: phaseWeekNow, rule };
}
