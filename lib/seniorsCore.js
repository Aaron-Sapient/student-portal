...
// usage already booked against it? Essential is budget-based (40-min weekly
...
// How many more meetings the grant can fund (Essential expressed as 20-min slots,
// matching the existing display). `usage` = { count, minutes } of active bookings.
export function grantRemaining(grant, usage) {
  if (!grant) return 0;
  if (grant.budget_minutes != null) {
    return Math.max(0, Math.floor((grant.budget_minutes - (usage.minutes || 0)) / 20));
  }
  return Math.max(0, grant.meeting_tokens - (usage.count || 0));
}

// THE unified senior booking authorization (pure). `state` = { grant, bookings }
// where bookings is the active consumption rows for that grant
// ([{ teacher, minutes, meeting_date }]). Layers the four grant gates (active,
// window, same-day, tokens) over the unchanged per-meeting-date package rules in
// canBook (teacher / length / phase secondary-first). Returns { ok, reason }.
export function canBookOnDate(senior, dt, teacherSlug, mins, state) {
  ...
  if (!grantHasRoom(grant, usage, mins)) return { ok: false, reason: 'tokens-used' };
  // Per-meeting-date package rules (teacher / duration / phase secondary-first),
  // evaluated against what's already booked in THAT meeting's Saturday-week.
  const wsKey = startOfSaturdayWeek(dt).toISODate();
  const weekBooked = emptyWeek();
  ...
  return canBook(senior, dt, teacherSlug, mins, weekBooked);
}
