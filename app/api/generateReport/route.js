...
async function fetchStudentData(sheets, studentSheetId, studentName) {
  ...
  const year     = ov[2]?.[1] || '';   // C4
  const major    = ov[4]?.[1] || '';   // C6
  const sat      = ov[15]?.[1] || '';  // D17
  const numAPs   = ov[16]?.[1] || '';  // D18

  // Active/completed projects with current/future deadlines
  ...
  // Summer coursework
  const summerRows = transcriptRes.data.values || [];
  const summerCourses = summerRows
    .filter(r => r[0])
    .map(r => ({
      course: r[0] || '',
      institution: r[1] || '',
...
async function fetchGender(sheets, studentName) {
  ...
  const rows = res.data.values || [];
  // Col A = index 0 (name), Col AX = index 49
...
function buildHtmlEmail(reportMarkdown, studentName) {
  // Convert simple markdown-like structure to HTML
  // The AI will use ## for H1 sections and ### for H2 sections
...
async function sendReportEmail(studentName, reportMarkdown) {
  ...
  const html = buildHtmlEmail(reportMarkdown, studentName);
  const text = buildPlainText(reportMarkdown, studentName);
...
export async function POST(request) {
  try {
    ...
    if (!studentName || !studentSheetId) {
      return Response.json({ error: 'Missing studentName or studentSheetId' }, { status: 400 });
    }
    ...
    if (!checkinHistory) {
      console.error('generateReport: no checkin history found for', studentName);
      return Response.json({ error: 'No checkin history found' }, { status: 404 });
    }
...
`.trim();
    ...
    const systemPrompt = `You are a private admissions counselor writing a weekly check-in report for a student. Write in professional but warm prose. Use the student's correct pronouns throughout.

CRITICAL RULES:
- Never infer, fill in gaps, or hallucinate. Use only the imported data as your source of truth.
- If something is missing or unclear, omit it entirely rather than speculating.
- Do not recommend academic tutoring services (outside scope). SAT tutoring is acceptable.
- Be concise — this is a 1-page report.
...
## STRATEGY
### Short-Term
1–3 short digestible priorities for the next few weeks.
### Long-Term
1–2 longer-horizon priorities.

## RECOMMENDATIONS
1–3 short, actionable items for the next 1–2 weeks.
