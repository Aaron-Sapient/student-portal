...
// TIME LOGIC: the grades/transcript step never shows June–August. The server
// already signals this (getCurrentSemester → 'NA' for months 6–8, so no classes
// are returned), and we guard again client-side so the step is impossible to reach
// in summer even if the payload changes.
function isSummer() {
...
// One pill in a segmented selector.
...
export default function RyanCheckIn() {
  ...
  const [feel, setFeel] = useState(50); // 0–100 clay slider; collapses to 1–10 on submit
  ...
  // Grades step shows only with real classes, a HS grade year, an active semester,
  // and never in summer (June–August).
  const showGrades =
    !!formData &&
    !formData.skip &&
    ...
    return (
      <div className="portal-rise flex min-h-[55vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-xs text-sm text-ink-soft">
          Ryan has this week’s update. The next check-in opens Saturday.
        </p>
      </div>
    );
  }
  ...
  if (status === 'routed') {
    return (
      <div className="portal-rise flex min-h-[55vh] flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-xs text-sm text-ink-soft">
          Ryan will follow up with a written update soon — no meeting needed this week.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-2">
      ...
      <div
        key={current}
        className="portal-rise neu-raised relative overflow-hidden rounded-3xl p-6 sm:p-7"
      >
        {current === 'week' && (
          <>
            <StepHeader
        ...
        {current === 'concerns' && (
          <>
            <StepHeader
              ...
              blurb="Anything you want Ryan to know about this week?"
            />
